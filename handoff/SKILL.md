...
| `--harvest` | run the step-5 knowledge harvest unconditionally after writing the handoff — skips the ask, not the gates (curate-vault's per-article confirmation still applies). For milestone checkpoints: `/handoff --harvest` |
...
- Composition by reference — complements the file-based memory system (durable facts), CLAUDE.md (project rules), and `curate-vault` (durable insights → the OKF vault; offered at the session boundary in step 5); `HANDOFF.md` holds *this task's* working state. Setup of the status line + hooks is delegated to `update-config` / `statusline-setup` (see `references/setup.md`).
