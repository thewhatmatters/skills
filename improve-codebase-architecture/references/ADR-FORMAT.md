...
Create the `docs/adr/` directory lazily — only when the first ADR is
needed.
...
- **Technology choices that carry lock-in.** Database, message bus, auth
  provider, deployment target. Not every library — just the ones that
...
- **Deliberate deviations from the obvious path.** "We're using manual SQL
  ...
  assume the opposite. These stop the next engineer from "fixing"
