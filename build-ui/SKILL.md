...
This skill owns **execution mechanics**. Aesthetic *direction* (bold, distinctive, anti-AI-slop) is `frontend-design`'s territory; animation as a craft will be `add-motion`'s (planned — not yet built). Compose by reference — don't reinvent.
...
3. **Load only the references the project uses** — first, if `--stack=KEY` was passed, override the matching probe field before routing: `--stack=tailwind` → `css=tailwind`; `--stack=vanilla-css` → `css=vanilla`; `--stack=shadcn` → `components=shadcn`. Then for each detected lib, read the matching `references/` file:
   ...
   - `framework == "next"` → branch on `external_skills["next-best-practices"]`:
     - **`true`** (the official skill is installed at `~/.claude/skills/next-best-practices`): defer **Next.js-specific patterns** (file conventions, RSC boundaries, async APIs in 15+, runtime selection, directives `'use client'`/`'use server'`/`'use cache'`, navigation hooks, error files, data patterns, route handlers, metadata + `next/og`, `next/image`, `next/font`, bundling, scripts) to it. Read [`references/next-best-practices.md`](references/next-best-practices.md) for build-ui's coordination layer (routing table; what build-ui still owns).
     - **`false`** (skill not installed): degraded path. Surface the install command: `npx skills add https://github.com/vercel-labs/next-skills --skill next-best-practices` (skills.sh; pinned in `skills-lock.json`). Until installed, fall back to general knowledge and **flag the caveat**: Next 15+ async params/`searchParams`/`cookies()`/`headers()`, the v16 `middleware` → `proxy` rename, and current canary conventions may not be in the training cutoff.
   - `components == "shadcn"` → branch on `external_skills.shadcn`:
     - **`true`** (the official skill is installed at `~/.claude/skills/shadcn`): defer **execution** (add, preset, registry, `info --json`, current primitives, smart-merge) to it. Read [`references/shadcn.md`](references/shadcn.md) for build-ui's coordination layer: the no-monoculture rule, how the probe and `info --json` are sequenced, and the routing table for when to invoke the shadcn skill vs. write JSX against installed primitives.
     - **`false`** (skill not installed on this machine — a fresh-clone case): degraded path. Surface the install command before doing the work: `npx skills add https://github.com/shadcn/ui --skill shadcn` (skills.sh; version pinned in `skills-lock.json`). Until the user installs it, fall back to general knowledge for the current task and **flag the caveat plainly**: live `info --json`, presets, and recent primitives (`FieldGroup`, `InputGroup`, `ToggleGroup`, …) may not match the project's current shadcn version. Still consult `references/shadcn.md` for the no-monoculture rule + routing intent.
