...
3. **Load only the references the project uses** — first, if `--stack=KEY` was passed, override the matching probe field before routing: `--stack=tailwind` → `css=tailwind`; `--stack=vanilla-css` → `css=vanilla`; `--stack=shadcn` → `components=shadcn`. Then for each detected lib, read the matching `references/` file:
   ...
   - The ask names a **grid system** / Müller-Brockmann / Swiss / column+baseline layout (e.g. "build the blog on a grid system") → **establish the grid spine first with the sibling `use-grid-system` skill** (it emits the `@theme`/`:root` grid source of truth + the band/overlay patterns and owns the profile choice + adherence verification), then execute here **placing elements by column line** on those tokens. build-ui doesn't own grid discipline — compose, don't reinvent.
...
- **Composition by reference, not import** (spec A8): `frontend-design` owns aesthetic direction; `add-motion` owns animation craft; `source-ui` finds visual precedent; `use-grid-system` owns Müller-Brockmann grid discipline (the grid source of truth, column-line placement, baseline, overlay, adherence verification). build-ui does *execution*. Compose by naming, not by including their logic.
