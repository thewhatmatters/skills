...
3. **Load only the references the project uses** — for each detected lib, read the matching `references/` file:
   ...
   - `components == "shadcn"` → defer the **execution** surface (add components, presets, registry search, live project info, current primitive set) to the **official `shadcn` skill** (auto-triggers on shadcn projects; pinned in `skills-lock.json`). Read [`references/shadcn.md`](references/shadcn.md) for the *build-ui complement*: no-monoculture rule, project-probe alignment, compose-don't-fork, routing table for when to invoke the shadcn skill vs. when to write JSX directly against installed primitives.
