...
- **Composes with `/trendscan`, doesn't import it.** When the research
  question has a recency angle ("what are people saying lately about X"),
  the skill's Step 4 invokes `/trendscan` via standard skill composition for
...
- **`KEYS_MISSING` is a Recoverable Setup Gate but NEVER blocks.** The gate
  ...
  This contrasts with trendscan's tighter coupling to specific providers.
- **Anti-overtrigger guard at Step 1.** The description is intentionally
  ...
  fact lookups and suggests `/trendscan` when the real question is about
  recent discussion.
...
- **No clobber on output filenames.** Re-running on the same slug writes
  ...
  filenames, same convention as `prd-generator`).
- **No DDG fallback inside `search.py`.** When neither Tavily nor Exa is
  ...
  focused and avoids the bot-block fragility trendscan's handoff §6
  documents on the dev machine's DDG endpoint.

## 3. Decision log
- 2026-05-19: scaffolded by skill-generator (formal `/skill-generator`
  invocation).
...
- 2026-05-19: self-audited via skill-auditor (--agent): 0 critical, 1
  ...
  - 🟡 shared `.env.example` "Used by:" notes were stale → updated to list
    `trendscan, deep-research` for TAVILY/EXA (done; see §6).
...
- Search.py uses `requests` (already in the environment via trendscan); no
...
## 6. Notes
Composes with `/trendscan` for recency; uses Tavily/Exa via the shared
...
notes now read `trendscan, deep-research` (updated 2026-05-19 as part of the
