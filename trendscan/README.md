...
```
/trendscan <topic> [--days=N] [--sources=a,b,c | --all] [--agent] [--out=PATH]
```

Examples:

- `/trendscan "AI browser agents"` — last 30 days, you pick sources
- `/trendscan "vision pro" --days=7 --all` — last week, every source
- `/trendscan "rust async" --agent` — unattended, no prompts
