# trendscan — Handoff & Skill Architecture

Living record of what trendscan is, the reusable patterns it established, why
decisions were made, and a rubric usable by a future **skill-generator** and
**skill-auditor**.
...
- **Install:** symlink `~/.claude/skills/trendscan` → `~/Downloads/trendscan`
...
- **`/last30days` → `/trendscan`** + `--days=N`: a fixed "30 days" in the name
...
- **Flat script names kept:** prefixing is redundant with the `trendscan/scripts/`
...
- Run: `/trendscan <topic> [--days=N] [--sources=a,b,c|--all] [--agent] [--out=PATH]`
...
- Editing the skill: edit `~/Downloads/trendscan/*`; live via symlink; SKILL.md
