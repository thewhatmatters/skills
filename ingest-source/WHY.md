...
`docs/sources/` with an INDEX.md that the project's AGENTS.md `@`-imports.
...
- **A7:** three gates — yt-dlp install (absorbed), Gemini key (degrade), and
  the new **AGENTS.md wiring gate** (consent before editing the user's file).
...
- 2026-07-02: **Added the destination gate (step 5) — project / vault / both.**
  ...
  scopes — project-local (docs/sources/ + AGENTS.md import) vs cross-project
  (OKF vault).
...
- 2026-06-11: **Persistence is the point; summarizing is the means.** The new
  capability vs summarize-yt is `docs/sources/` + INDEX.md + the AGENTS.md
  `@`-import. AGENTS.md cannot "watch a folder" — `@docs/sources/INDEX.md`
...
- 2026-06-11: **Scripts never edit AGENTS.md.** `persist.py` only reports
  wiring status + emits the canonical block; the edit is performed natively by
  the agent behind a consent gate (interactive ask; `--agent` skips + prints the
  block). Editing a user's AGENTS.md from a subprocess would bypass the
...
- 2026-06-11: **Webpage extraction is stdlib `html.parser`**, not readability/
  trafilatura (stdlib-first rule). Good enough because the agent re-reads the
...
- Carried from summarize-yt (2026-05-24): The agent can't watch video natively →
...
- `fetch_url.py` extraction is heuristic — SPAs and consent-walled pages
  ...
  parser.
- The AGENTS.md pointer inlines INDEX.md into every session of the target
...
preflight / binary-resolution rows all apply; the AGENTS.md gate is the
