...
**What it is:** turns any source — a YouTube video, webpage, PDF, image, or document — into a structured, cited markdown summary, and saves it as project knowledge your AGENTS.md can see. (Replaces `summarize-yt`.)
...
- One-time, with your OK: a small block in your project's AGENTS.md that `@`-imports the index — so every future session starts knowing what sources the project has captured, and reads the full files only when relevant.
...
- **Optional:** a Google API key (`GOOGLE_API_KEY` or `GEMINI_API_KEY`) in the shared `~/.cursor/skills/.env` unlocks the YouTube **watch** tier — Gemini sees the screen, not just the soundtrack, which matters for on-screen text and demos. No key → captions, never blocked.
...
5. Asks once per project whether to wire the index into AGENTS.md; you can also paste the printed block yourself.
