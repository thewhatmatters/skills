...
4. **Archive check, then write** — *before* writing to `--out`: if it already holds a `prd.json` for a *different* `branchName` with real progress, follow the archive convention in `references/conversion-rules.md`. Only then write the new `prd.json`.
5. **Validate** — `python3 scripts/validate.py --in=<out>` (unless `--no-validate`). Fix every reported error before finalizing.
