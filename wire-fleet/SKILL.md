...
Bolt the agent-fleet `.fleet/` overlay onto an existing project and wire its
...
"wire this project to the fleet", "install .fleet here", or
`/wire-fleet [--project=PATH]`. Refresh with `/wire-fleet --update`.
...
location, git status, existing `.fleet/` (+ pinned version), detected gate
...
Copies the pinned template into `.fleet/`, substitutes project config into
fleet.yaml, writes the template manifest (hashes) for future `--update`,
merges the commit-policy lines into the project's `.gitignore`. Refuses to
clobber an existing `.fleet/` (that's `--update`'s job).
...
version, and the un-wire line (`delete .fleet/ + remove the marker block`).
Suggest the first ticket: copy `.fleet/tickets/T-000-example.md`.
...
- Writes outside `.fleet/` are limited to the marker block (consent-gated,
