...
- Template bundled under `assets/template/` rather than referenced from
  `~/Development/agent-fleet` so the skill is self-contained and the template
...
- 2026-08-06: template synced from the `~/Development/agent-fleet` v0 test
  bed (T-001 run), with paths renamed `.agent/` → `.fleet/` per the bolt-on
...
- 2026-08-06: `--update` detects local modification via a sha256 manifest
  written at install (`.fleet/.template-manifest.json`); modified files are
...
- Runtime enforcement of fleet.yaml (round caps, gates) is NOT this skill's
  job and does not exist yet anywhere — the installed fleet.yaml says so
