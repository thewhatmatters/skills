...
already follows our house conventions, then has `skill-auditor` review it —
...
Plus the audit verdict from `skill-auditor` on what was just generated, so
...
5. **Self-audit** — run `skill-auditor` on the new skill and attach its
...
- `DESIGN.md` — design decisions, the resolution chain, and what each script
  does in detail (living record, like trendscan's `handoff.md`).
...
- `../skill-auditor/` — the counterpart that *reviews* skills against the
  same spec.
