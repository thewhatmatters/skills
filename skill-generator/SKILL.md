...
description: Scaffold a new Claude Code skill against the house spec at ~/.claude/skills/skill-architecture.md, then self-audit it. Use when the user wants to create, scaffold, generate, bootstrap, or seed a new skill — "create a new skill", "make me a skill that …", "scaffold a Claude skill", "generate a skill that does X", "skeleton a skill", "spin up a skill", "I want a skill for Y". Pulls the live Claude docs (offline fallback), validates frontmatter against the upstream field list, follows the 13 architecture patterns, and runs skill-auditor on the output. Reports honestly — never claims success when the auditor finds high-severity issues.
---
...
runs `skill-auditor` on what was just produced.
...
Invoke the **skill-auditor** skill on the new skill directory:
