...
Say something like "make an HTML version of `report.md`" or run `/html-output report.md`. By default the HTML lands next to the input as `report.html`. You can add a custom title (`--title="Q2 Review"`), print to stdout instead (`--stdout`), or force fully-offline output with `--no-webfonts`.
